...
class IntnotificationCron extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'intnotification:cron';
    ...
    {
        $clients = InterviewListModel::where('checkflag',1)->whereNotIn('cchk',[1,2])->select('client')->get();
        $providers = InterviewListModel::where('checkflag',1)->whereNotIn('pchk',[1,2])->select('provider')->get();
        ...
        for($i = 0;$i < count($clients); $i ++){
            $client[$i] = $clients[$i]->client;
        }
        for($i = 0;$i < count($providers); $i ++){
            $provider[$i] = $providers[$i]->provider;
        }
